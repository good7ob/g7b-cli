 ...
 */
export function registerAgentReportCommands(logCommand: Command) {
  ...
  reportCommand
    ...
    .action(async (options) => {
      try {
        ...
        if (options.userId) body.userId = parseInt(options.userId);
        if (options.projectId) body.projectId = parseInt(options.projectId);
        if (options.periodStart) body.periodStart = options.periodStart;
        if (options.periodEnd) body.periodEnd = options.periodEnd;

        const report = await apiClient.post(`${API_PREFIX}/generate`, body);
        if (options.json) {
  ...
  reportCommand
    ...
    .action(async (options) => {
      try {
        ...
        if (options.scope) params.scopeType = options.scope;
        if (options.userId) params.userId = parseInt(options.userId);
        if (options.projectId) params.projectId = parseInt(options.projectId);
        if (options.type) params.reportType = options.type;

        const result = await apiClient.get(API_PREFIX, params);
        if (options.json) {
  ...
  reportCommand
    ...
    .action(async (id, options) => {
      try {
        const report = await apiClient.get(`${API_PREFIX}/${id}`);
        if (options.json) {
          console.log(JSON.stringify(report, null, 2));
          return;
        }
  ...
  reportCommand
    ...
    .action(async (id, options) => {
      try {
        ...
        if (options.contentFile) body.content = readFileSync(options.contentFile, 'utf8');

        const report = await apiClient.put(`${API_PREFIX}/${id}`, body);
        if (options.json) {
          console.log(JSON.stringify(report, null, 2));
          return;
        }
