...
export function registerLogCommands(program: Command) {
  ...
  logCommand
    ...
    .action(async (options) => {
      try {
        ...
        if (options.input) body.inputSummary = options.input;
        if (options.error) body.errorMessage = options.error;
        if (options.tokens) body.tokenUsage = parseInt(options.tokens);
        if (options.duration) body.durationMs = parseInt(options.duration);
        if (options.model) body.modelVersion = options.model;
        if (options.sessionId) body.sessionId = options.sessionId;
        if (options.parentId) body.parentId = parseInt(options.parentId);
        if (options.tags) body.tags = options.tags;
        if (options.remark) body.remark = options.remark;

        const record = await apiClient.post('/forge/ai-work-record', body);
        if (options.json) {
  ...
  logCommand
    ...
    .action(async (options) => {
      try {
        ...
        if (options.projectId) body.projectId = parseInt(options.projectId);
        if (options.taskId) body.taskId = parseInt(options.taskId);
        if (options.agent) body.aiAgent = options.agent;
        if (options.taskType) body.taskType = options.taskType;
        if (options.status) body.status = options.status;
        if (options.result) body.result = options.result;
        if (options.search) body.keyword = options.search;
  ...
  logCommand
    ...
    .action(async (id, options) => {
      try {
        const record = await apiClient.get(`/forge/ai-work-record/${id}`);
        if (options.json) {
          console.log(JSON.stringify(record, null, 2));
          return;
        }
  ...
  logCommand
    ...
    .action(async (id, options) => {
      try {
        ...
        await apiClient.delete(`/forge/ai-work-record/${id}`);
  ...
  logCommand
    ...
    .action(async (options) => {
      try {
        ...
        await apiClient.post('/forge/ai-work-record/batch-delete', ids);
  ...
  logCommand
    ...
    .action(async (options) => {
      try {
        ...
        const stats = await apiClient.get('/forge/ai-work-record/statistics', params);
        if (options.json) {
