...
export function registerProductCommands(orgCommand: Command) {
  // list products
  orgCommand
    ...
    .action(async (orgId, options) => {
      try {
        ...
        const result = await apiClient.get(`/api/v1/orgs/${orgId}/products`, params);
  ...
  orgCommand
    ...
    .action(async (orgId, productId, options) => {
      try {
        const product = await apiClient.get(`/api/v1/orgs/${orgId}/products/${productId}`);
        if (options.json) {
  ...
  orgCommand
    ...
    .action(async (orgId, options) => {
      try {
        ...
        if (options.techTags) body.techTags = options.techTags.split(',').map((t: string) => t.trim());
        if (options.coverUrl) body.coverUrl = options.coverUrl;

        const product = await apiClient.post(`/api/v1/orgs/${orgId}/products`, body);
        if (options.json) {
          console.log(JSON.stringify(product, null, 2));
          return;
        }
  ...
  orgCommand
    ...
    .action(async (orgId, productId, options) => {
      try {
        ...
        await apiClient.put(`/api/v1/orgs/${orgId}/products/${productId}`, body);
  ...
  orgCommand
    ...
    .action(async (orgId, productId, options) => {
      try {
        ...
        await apiClient.delete(`/api/v1/orgs/${orgId}/products/${productId}?action=${action}`);
