...
function resolveProductId(optionValue?: string): number {
  const raw = optionValue || process.env.GOOD7OB_PRODUCT_ID;
  const id = raw ? parseInt(raw, 10) : NaN;
  if (!id || Number.isNaN(id)) {
    throw new Error(
      '缺少产品 ID。用 --product <id> 指定，或设置环境变量 GOOD7OB_PRODUCT_ID。\n' +
 ...
 */
async function resolveId(input: string, productOption?: string): Promise<number> {
  const direct = /^[0-9]+$/.exec(input.trim());
  if (direct) return parseInt(input, 10);
...
export function registerReqCommands(program: Command) {
  ...
  reqCommand
    ...
    .action(async (titleParts: string[], options) => {
      try {
        ...
        const payload: any = { productId, title: titleParts.join(' ') };
        if (options.description) payload.description = options.description;
        if (options.source) payload.source = options.source;
        if (options.sourceDetail) payload.sourceDetail = options.sourceDetail;
        if (options.parent) payload.parentId = parseInt(options.parent, 10);

        const created = await apiClient.post('/forge/requirements', payload);
        if (options.json) {
      ...
      try {
        ...
        if (options.csv) {
          ...
          records.forEach((r: any) => {
            ...
            console.log(
              `${reqNo(r)},${r.id},${esc(r.title)},${r.status || ''},${r.source || ''},` +
        ...
        records.forEach((r: any) => {
          ...
          if (isClosed) {
            ...
            if (r.closeNote) console.log(' '.repeat(10) + `└ ${r.closeNote}`);
          } else {
  ...
  reqCommand
    ...
    .action(async (id, options) => {
      try {
        ...
        const r = await apiClient.get(`/forge/requirements/${rid}`);
        if (options.json) {
  ...
  reqCommand
    ...
    .action(async (id, options) => {
      try {
        const rid = await resolveId(id, options.product);
        const moved = await apiClient.post(`/forge/requirements/${rid}/status`, {
          toStatus: 'active',
  ...
  reqCommand
    ...
    .action(async (id, options) => {
      try {
        const rid = await resolveId(id, options.product);
        const closed = await apiClient.post(`/forge/requirements/${rid}/close`, {
  ...
  reqCommand
    ...
    .action(async (id, options) => {
      try {
        ...
        const rid = await resolveId(id, options.product);
        const updated = await apiClient.put(`/forge/requirements/${rid}`, payload);
  ...
  reqCommand
    ...
    .action(async (id, options) => {
      try {
        const rid = await resolveId(id, options?.product);
        await apiClient.delete(`/forge/requirements/${rid}`);
    ...
    .action(async (id, options) => {
      try {
        ...
        const [versions, logs] = await Promise.all([
          apiClient.get(`/forge/requirements/${rid}/versions`).catch(() => null),
          apiClient.get(`/forge/requirements/${rid}/state-logs`).catch(() => null),
        ]);
        ...
        if (Array.isArray(logs) && logs.length) {
          ...
          logs.forEach((l: any) => {
            console.log(
              `${(STATUS_LABEL[l.fromStatus] || l.fromStatus || '新建').padEnd(8)}→ ` +
