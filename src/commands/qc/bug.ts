...
export function registerBugCommands(qcCommand: Command) {
  ...
  bugCmd
    ...
    .action(async (projectId, options) => {
      try {
        ...
        if (options.status)   body.statuses   = options.status.split(',');
        if (options.severity) body.severities  = options.severity.split(',');
        if (options.module)   body.moduleName  = options.module;
  ...
  bugCmd
    ...
    .action(async (id, options) => {
      try {
        const result = await apiClient.get(`/qc/bugs/${id}`);
        if (options.json) { console.log(JSON.stringify(result, null, 2)); return; }

        const b: any       = result?.bug ?? result;
        ...
        if (b.firstResolvedAt) console.log(`首次解决:     ${b.firstResolvedAt}`);
        if (b.finalResolvedAt) console.log(`最终解决:     ${b.finalResolvedAt}`);
        if (b.closedAt)        console.log(`关闭时间:     ${b.closedAt}`);
        if (b.reasonType)      console.log(`原因分类:     ${b.reasonType}`);
        if (b.rootCause)       console.log(`根本原因:     ${b.rootCause}`);
  ...
  bugCmd
    ...
    .action(async (options) => {
      try {
        ...
        if (options.stage)  body.foundStage  = options.stage;
        if (options.source) body.sourceType  = options.source;

        const result = await apiClient.post('/qc/bugs', body);
        if (options.json) { console.log(JSON.stringify(result, null, 2)); return; }
  ...
  bugCmd
    ...
    .action(async (id, targetStatus, options) => {
      try {
        const body: Record<string, unknown> = { targetStatus };
        if (options.comment)    body.comment    = options.comment;
        if (options.reasonType) body.reasonType = options.reasonType;
        if (options.rootCause)  body.rootCause  = options.rootCause;
        if (options.assignTo)   body.assignedTo = parseInt(options.assignTo);
        if (options.developerId) body.developerId = parseInt(options.developerId);

        const result = await apiClient.post(`/qc/bugs/${id}/transition`, body);
        if (options.json) { console.log(JSON.stringify(result, null, 2)); return; }
  ...
  bugCmd
    ...
    .action(async (id, options) => {
      try {
        const body: Record<string, unknown> = {};
        if (options.assignedTo)  body.assignedTo  = parseInt(options.assignedTo);
        if (options.developerId) body.developerId = parseInt(options.developerId);
        if (options.testerId)    body.testerId    = parseInt(options.testerId);

        await apiClient.post(`/qc/bugs/${id}/assign`, body);
