...
export function registerWorkflowCommands(pmCommand: Command) {
  ...
  workflowCommand
    ...
    .action(async (options) => {
      try {
        const templates = await apiClient.get('/progress/workflows/templates');
        if (options.json) {
  ...
  workflowCommand
    ...
    .action(async (id, options) => {
      try {
        const template = await apiClient.get(`/progress/workflows/templates/${id}`);
        if (options.json) {
  ...
  workflowCommand
    ...
    .action(async (options) => {
      try {
        await apiClient.post('/progress/workflows/start', {
  ...
  workflowCommand
    ...
    .action(async (taskId, options) => {
      try {
        ...
        await apiClient.post(`/progress/workflows/${taskId}/stages/complete`, body);
  ...
  workflowCommand
    ...
    .action(async (taskId, options) => {
      try {
        const action = await apiClient.get(`/progress/workflows/${taskId}/next-action`);
        if (options.json) {
  ...
  workflowCommand
    ...
    .action(async (projectId, options) => {
      try {
        const statuses = await apiClient.get(`/progress/workflows/project/${projectId}`);
        if (options.json) {
        ...
        list.forEach((s: any) => {
          console.log(
            String(s.taskId || s.id).padEnd(10) +
  ...
  workflowCommand
    ...
    .action(async (options) => {
      try {
        await apiClient.post('/progress/workflows/dependencies', {
          taskId: parseInt(options.taskId),
  ...
  workflowCommand
    ...
    .action(async (taskId, options) => {
      try {
        const deps = await apiClient.get(`/progress/workflows/dependencies/${taskId}`);
        if (options.json) {
        ...
        list.forEach((d: any) => {
          console.log(
            String(d.dependsOnTaskId || d.taskId).padEnd(12) +
