...
export function registerKbCommands(program: Command) {
  ...
  kb
    ...
    .action(async (options) => {
      try {
        ...
        const docs: any[] = (await apiClient.get('/forge/kb/documents', params)) ?? [];
        const keyword = options.name?.toLowerCase();
        const records = docs.filter(
          (d) =>
            (!keyword || String(d.name ?? '').toLowerCase().includes(keyword)) &&
  ...
  kb
    ...
    .action(async (queryParts: string[], options) => {
      try {
        ...
        const result = await apiClient.get('/forge/kb/search', {
          q,
          limit: parseInt(options.limit, 10) || 10,
        });
        const hits: any[] = result?.results ?? [];
  ...
  kb
    ...
    .action(async (messageParts: string[], options) => {
      try {
        const stream = await apiClient.postStream('/forge/kb/chat', { message: messageParts.join(' ') });
        ...
        if (options.citations && final.citations.length) {
          console.log('\n引用:');
          final.citations.forEach((c: any, i: number) => {
...
function consumeChat(stream: NodeJS.ReadableStream, onText: (t: string) => void): Promise<ChatResult> {
  return new Promise((resolve, reject) => {
    const result: ChatResult = { answer: '', citations: [] };
    let buffer = '';
    let settled = false;
    ...
    stream.on('data', (chunk: string) => {
      ...
      for (const ev of parsed.events) {
        ...
        if (ev.event === 'error') return finish(new Error(payload.error || ev.data));
        if (payload.content) {
