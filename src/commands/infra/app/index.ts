...
export function registerAppCommands(infraCommand: Command) {
  ...
  appCommand
    ...
    .action(async (options) => {
      try {
        ...
        const result = await apiClient.post('/api/infra/applications', payload);
  ...
  appCommand
    ...
    .action(async (appId, options) => {
      try {
        ...
        if (options.ownerId) payload.ownerId = parseInt(options.ownerId);
        if (options.coOwnerId) payload.coOwnerId = parseInt(options.coOwnerId);
        if (options.description) payload.description = options.description;
        if (options.tags) payload.tags = options.tags.split(',').map((t: string) => t.trim());
        if (options.techStack) payload.techStack = options.techStack;
        if (options.archType) payload.archType = options.archType;
        if (options.deps) payload.dependencies = options.deps;
        if (options.status) payload.status = options.status;

        await apiClient.put(`/api/infra/applications/${appId}`, payload);
  ...
  appCommand
    ...
    .action(async (appId, options) => {
      try {
        ...
        await apiClient.delete(`/api/infra/applications/${appId}`);
  ...
  appCommand
    ...
    .action(async (appId, options) => {
      try {
        const app = await apiClient.get(`/api/infra/applications/${appId}`);
        ...
        if (options.json) {
          ...
          if (app.tags) console.log(`Tags: ${app.tags}`);
          if (app.techStack) console.log(`Tech Stack: ${app.techStack}`);
        }

        if (options.showResources) {
          const resources = await apiClient.get(`/api/infra/applications/${appId}/resources`);
  ...
  appCommand
    ...
    .action(async (options) => {
      try {
        ...
        const result = await apiClient.post('/api/infra/applications/list', params);
      ...
      try {
        ...
        for (let i = 0; i < applications.length; i++) {
          try {
            ...
            await apiClient.post('/api/infra/applications', app);
            successCount++;
      ...
      try {
        ...
        if (options.includeResources) {
          ...
          for (const app of applications) {
            try {
              const resources = await apiClient.get(`/api/infra/applications/${app.id}/resources`);
  ...
  appCommand
    ...
    .action(async (appId, options) => {
      try {
        ...
        if (options.replace) {
          // Replace all tags
          tags = options.replace.split(',').map((t: string) => t.trim());
          ...
          // Remove tags
          const removeTags = options.remove.split(',').map((t: string) => t.trim());
  ...
  appCommand
    ...
    .action(async (appId, options) => {
      try {
        ...
        await apiClient.post(`/api/infra/applications/${appId}/resources`, resourceIds);
  ...
  appCommand
    ...
    .action(async (appId, resourceId, options) => {
      try {
        ...
        await apiClient.delete(`/api/infra/applications/${appId}/resources/${rid}`);
        console.log(`✓ 成功解绑资源`);
      } catch (error) {
        ...
        try {
          ...
          if (options.checkIncomplete) {
            ...
            allApps.forEach((app: any) => {
              const incomplete = [];
              if (!app.description) incomplete.push('description');
              if (!app.techStack) incomplete.push('tech_stack');
              if (!app.archType) incomplete.push('architecture');
