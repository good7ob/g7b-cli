...
function registerResourceCommands(infraCommand) {
    ...
    resourceCommand
        ...
        try {
            ...
            if (options.type)
                params.resourceType = options.type;
            if (options.provider)
            ...
            if (options.json) {
                ...
                result.records?.forEach((res) => {
                    console.log(`${res.id},${res.resourceId},"${res.resourceName}",${res.resourceType},${res.cloudProvider},${res.environment},${res.status},${res.monthlyCost}`);
                });
            }
            else {
    ...
    resourceCommand
        ...
        try {
            const resource = await ApiClient_1.default.get(`/api/infra/resources/${resourceId}`);
            if (options.json) {
            ...
            if (options.showCosts) {
                try {
                    const costHistory = await ApiClient_1.default.get(`/api/infra/resources/${resourceId}/cost-history`);
                    console.log('\n成本历史 (最近6个月):');
                    console.log('─'.repeat(50));
                    costHistory.records?.slice(0, 6).forEach((record) => {
            ...
            if (options.showMetrics) {
                try {
                    const metrics = (await ApiClient_1.default.get(`/api/infra/resources/${resourceId}/metrics`));
                    console.log('\n性能指标:');
                    console.log('─'.repeat(50));
            ...
            if (options.showApps) {
                try {
                    const apps = (await ApiClient_1.default.get(`/api/infra/resources/${resourceId}/apps`));
                    console.log('\n关联应用:');
                    console.log('─'.repeat(50));
    ...
    resourceCommand
        ...
        try {
            ...
            try {
                ...
                const result = (await ApiClient_1.default.post('/api/infra/resources/import', payload));
                console.log('─'.repeat(50));
                console.log(`✓ 导入完成`);
