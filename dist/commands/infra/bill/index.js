...
function registerBillCommands(infraCommand) {
    ...
    billCommand
        ...
        try {
            ...
            const result = await ApiClient_1.default.post('/api/infra/bills/import', {
                fileContent,
                ...params,
            });
    ...
    billCommand
        ...
        try {
            ...
            if (options.provider)
                params.cloudProvider = options.provider;
            if (options.month)
                params.month = options.month;
            if (options.status)
                params.status = options.status;
            const result = await ApiClient_1.default.get('/api/infra/bills', params);
            if (options.json) {
                ...
                result.records?.forEach((bill) => {
                    console.log(`${bill.id},${bill.month},${bill.cloudProvider},${bill.status},${bill.recordCount},${bill.totalCost},${bill.createdAt}`);
                });
            }
            else {
    ...
    billCommand
        ...
        try {
            ...
            if (options.showReconcile)
                params.includeReconcile = true;
            if (options.limit)
                params.limit = parseInt(options.limit);
            const bill = await ApiClient_1.default.get(`/api/infra/bills/${billId}`, params);
            if (options.json) {
                ...
                if (options.showErrors && bill.errors && bill.errors.length > 0) {
                    console.log('\n导入错误:');
                    bill.errors.slice(0, parseInt(options.limit) || 100).forEach((error, idx) => {
                ...
                if (options.showRecords && bill.records && bill.records.length > 0) {
                    ...
                    bill.records.slice(0, parseInt(options.limit) || 100).forEach((record) => {
                        console.log(`${record.resourceId || '-'} | ${record.cost || 0} | ${record.date || '-'}`);
                    });
                }
            }
        }
    ...
    billCommand
        ...
        try {
            ...
            if (options.listSchedules) {
                const schedules = await ApiClient_1.default.get('/api/infra/bill-schedules');
                if (options.json) {
            ...
            if (options.removeSchedule) {
                await ApiClient_1.default.delete(`/api/infra/bill-schedules/${options.removeSchedule}`);
                console.log(`✓ 删除计划成功`);
                return;
            }
            ...
            if (options.addSchedule || options.scheduleId) {
                ...
                if (options.scheduleId) {
                    result = await ApiClient_1.default.put(`/api/infra/bill-schedules/${options.scheduleId}`, payload);
                    console.log(`✓ 更新计划成功`);
                }
                else {
                    result = await ApiClient_1.default.post('/api/infra/bill-schedules', payload);
                    console.log(`✓ 创建计划成功`);
                }
                if (options.json) {
                    console.log(JSON.stringify(result, null, 2));
            ...
            if (options.scheduleId && (options.enabled || options.disable)) {
                ...
                await ApiClient_1.default.patch(endpoint, {});
            ...
            if (options.test && options.scheduleId) {
                ...
                const result = await ApiClient_1.default.post(`/api/infra/bill-schedules/${options.scheduleId}/test`, {});
                if (options.json) {
                    console.log(JSON.stringify(result, null, 2));
                }
                else {
    ...
    billCommand
        ...
        try {
            // Get bill info first
            const bill = await ApiClient_1.default.get(`/api/infra/bills/${billId}`);
            if (!options.force) {
                console.log(`\n您即将删除账单 ${billId}`);
                console.log(`账单月份: ${bill.month}`);
                console.log(`云提供商: ${bill.cloudProvider}`);
            ...
            await ApiClient_1.default.delete(`/api/infra/bills/${billId}`, payload);
            console.log('✓ 账单已删除');
            if (options.hardDelete) {
...
exports.registerBillCommands = registerBillCommands;
