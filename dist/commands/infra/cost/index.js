...
function registerCostCommands(infraCommand) {
    ...
    costCommand
        ...
        try {
            ...
            const overview = await ApiClient_1.default.get('/api/infra/cost/overview', params);
            if (options.json) {
    ...
    costCommand
        ...
        try {
            ...
            const data = await ApiClient_1.default.get('/api/infra/cost/by-app', params);
            if (options.json) {
    ...
    costCommand
        ...
        try {
            ...
            const data = await ApiClient_1.default.get('/api/infra/cost/by-env', params);
            if (options.json) {
                console.log(JSON.stringify(data, null, 2));
            }
            else {
    ...
    costCommand
        ...
        try {
            ...
            const data = await ApiClient_1.default.get('/api/infra/cost/by-type', params);
            if (options.json) {
                console.log(JSON.stringify(data, null, 2));
            }
            else {
    ...
    costCommand
        ...
        try {
            ...
            const data = await ApiClient_1.default.get('/api/infra/cost/by-provider', params);
            if (options.json) {
                console.log(JSON.stringify(data, null, 2));
            }
            else {
    ...
    costCommand
        ...
        try {
            ...
            const data = await ApiClient_1.default.get('/api/infra/cost/trend', params);
            if (options.json) {
                console.log(JSON.stringify(data, null, 2));
            }
            else {
                ...
                if (data.trends && data.trends.length > 0) {
                    ...
                    const maxCost = Math.max(...data.trends.map((t) => t.cost || 0));
                    const scale = Math.ceil(maxCost / 20);
                    data.trends.forEach((trend) => {
                        const barLength = Math.round((trend.cost || 0) / scale);
                        const bar = '█'.repeat(barLength);
                        console.log(`${trend.month} │ ${bar} $${(trend.cost || 0).toFixed(2)}`);
                    });
                }
                console.log('─'.repeat(50));
    ...
    costCommand
        ...
        try {
            ...
            const data = await ApiClient_1.default.get('/api/infra/cost/waste', params);
            if (options.json) {
                console.log(JSON.stringify(data, null, 2));
            }
            else {
                ...
                if (data.wasteItems && data.wasteItems.length > 0) {
                    data.wasteItems.slice(0, limit).forEach((item) => {
    ...
    costCommand
        ...
        try {
            ...
            if (options.provider)
                params.cloudProvider = options.provider;
            const data = await ApiClient_1.default.get('/api/infra/cost/reconcile', params);
            if (options.json) {
                console.log(JSON.stringify(data, null, 2));
            }
            else {
                ...
                if (options.verbose && data.details && data.details.length > 0) {
                    ...
                    data.details.forEach((detail) => {
                        console.log(`  ${detail.category}: $${detail.difference?.toFixed(2)} (${detail.differencePercentage?.toFixed(1)}%)`);
                    });
                }
            }
        }
    ...
    costCommand
        ...
        try {
            ...
            const result = await ApiClient_1.default.get('/api/infra/cost/export', params);
    ...
    costCommand
        ...
        try {
            ...
            const data = await ApiClient_1.default.get('/api/infra/cost/forecast', params);
            if (options.json) {
                console.log(JSON.stringify(data, null, 2));
            }
            else {
                ...
                if (data.forecasts && data.forecasts.length > 0) {
                    data.forecasts.forEach((forecast) => {
                        ...
                        console.log(forecast.month.padEnd(15) +
