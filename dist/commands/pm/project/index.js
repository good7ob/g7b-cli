...
function registerProjectCommands(pmCommand) {
    ...
    projectCommand
        ...
        try {
            const project = await ApiClient_1.default.get(`/progress/projects/${id}`);
            if (options.json) {
    ...
    projectCommand
        ...
        .action(async (options) => {
        try {
            const body = { name: options.name, status: options.status };
            ...
            const project = await ApiClient_1.default.post('/progress/projects', body);
            if (options.json) {
                console.log(JSON.stringify(project, null, 2));
                return;
            }
    ...
    projectCommand
        ...
        try {
            ...
            await ApiClient_1.default.put(`/progress/projects/${id}`, body);
    ...
    projectCommand
        ...
        try {
            ...
            await ApiClient_1.default.delete(`/progress/projects/${id}`);
    ...
    projectCommand
        ...
        try {
            ...
            await ApiClient_1.default.post(`/progress/projects/${id}/archive?archived=${archived}`);
    ...
    projectCommand
        ...
        .action(async (id) => {
        try {
            const result = await ApiClient_1.default.post(`/progress/projects/${id}/recalculate`);
    ...
    projectCommand
        ...
        .action(async (id, options) => {
        try {
            const data = await ApiClient_1.default.get(`/progress/projects/${id}/gantt`);
            if (options.json) {
            ...
            console.log('─'.repeat(80));
            tasks.forEach((t) => {
                console.log(String(t.id).padEnd(10) +
    ...
    projectCommand
        ...
        .action(async (id, options) => {
        try {
            const participants = await ApiClient_1.default.get(`/progress/projects/${id}/participants`);
            if (options.json) {
    ...
    projectCommand
        ...
        try {
            const body = { userId: parseInt(options.userId) };
            if (options.displayName)
                body.displayName = options.displayName;
            if (options.role)
                body.role = options.role;
            await ApiClient_1.default.post(`/progress/projects/${projectId}/participants`, body);
    ...
    projectCommand
        ...
        try {
            ...
            await ApiClient_1.default.delete(`/progress/participants/${participantId}`);
