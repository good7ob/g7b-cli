...
async function resolveVersion(planId, explicit) {
    if (explicit !== undefined) {
        return parseInt(explicit, 10);
    }
    const plan = await ApiClient_1.default.get(`/progress/execution-plans/${planId}`);
...
function printItemsTable(items) {
    ...
    items.forEach((i) => {
        console.log(String(i.seq).padEnd(5) +
...
function printDetail(vo, options) {
    if (options.json) {
    ...
    if (vo.warnings && vo.warnings.length) {
        console.log('\n⚠ 警告');
        vo.warnings.forEach((w) => console.log('  - ' + w));
    }
}
...
function registerPlanCommands(pmCommand) {
    ...
    planCommand
        ...
        try {
            ...
            if (options.start)
                body.scheduledStartAt = parseDateTime(options.start);
            if (options.executionMode)
                body.executionMode = options.executionMode;
            if (options.tasks)
                body.taskIds = parseIdList(options.tasks);
            const vo = await ApiClient_1.default.post('/progress/execution-plans', body);
            if (options.json) {
                console.log(JSON.stringify(vo, null, 2));
                return;
            }
    ...
    planCommand
        ...
        try {
            const params = { projectId: parseInt(projectId, 10) };
            if (options.status)
                params.status = options.status;
            const result = await ApiClient_1.default.get('/progress/execution-plans', params);
    ...
    planCommand
        ...
        try {
            const vo = await ApiClient_1.default.get(`/progress/execution-plans/${id}`);
    ...
    planCommand
        ...
        try {
            ...
            if (options.estimatedMinutes)
                body.estimatedDurationMinutes = parseInt(options.estimatedMinutes, 10);
            if (options.executionMode)
                body.executionMode = options.executionMode;
            const vo = await ApiClient_1.default.put(`/progress/execution-plans/${id}`, body);
            if (options.json) {
                console.log(JSON.stringify(vo, null, 2));
                return;
            }
    ...
    planCommand
        ...
        .action(async (id) => {
        try {
            await ApiClient_1.default.delete(`/progress/execution-plans/${id}`);
    ...
    planCommand
        ...
        try {
            const version = await resolveVersion(id, options.version);
            const body = { taskId: parseInt(options.task, 10), version };
            if (options.after)
                body.afterItemId = parseInt(options.after, 10);
            const vo = await ApiClient_1.default.post(`/progress/execution-plans/${id}/items`, body);
            if (options.json) {
                console.log(JSON.stringify(vo, null, 2));
                return;
            }
    ...
    planCommand
        ...
        .action(async (id, options) => {
        try {
            const vo = await ApiClient_1.default.delete(`/progress/execution-plans/${id}/items/${options.item}`);
    ...
    planCommand
        ...
        try {
            ...
            const vo = await ApiClient_1.default.put(`/progress/execution-plans/${id}/items/reorder`, body);
            if (options.json) {
                console.log(JSON.stringify(vo, null, 2));
                return;
            }
    ...
    planCommand
        ...
        try {
            ...
            const vo = await ApiClient_1.default.post(`/progress/execution-plans/${id}/items/${options.item}/skip`, body);
    ...
    planCommand
        ...
        .action(async (id) => {
        try {
            const plan = await ApiClient_1.default.post(`/progress/execution-plans/${id}/schedule`);
    ...
    planCommand
        ...
        .action(async (id) => {
        try {
            const plan = await ApiClient_1.default.post(`/progress/execution-plans/${id}/start`);
    ...
    planCommand
        ...
        .action(async (id) => {
        try {
            const plan = await ApiClient_1.default.post(`/progress/execution-plans/${id}/pause`);
    ...
    planCommand
        ...
        try {
            const plan = await ApiClient_1.default.post(`/progress/execution-plans/${id}/halt`, {
                confirm: true,
    ...
    planCommand
        ...
        .action(async (id) => {
        try {
            const plan = await ApiClient_1.default.post(`/progress/execution-plans/${id}/resume`);
    ...
    planCommand
        ...
        .action(async (id) => {
        try {
            const plan = await ApiClient_1.default.post(`/progress/execution-plans/${id}/cancel`);
    ...
    planCommand
        ...
        .action(async (id, options) => {
        try {
            const result = await ApiClient_1.default.get(`/progress/execution-plans/${id}/events`);
            const records = (0, extractRecords_1.extractRecords)(result);
            if (options.json) {
                console.log(JSON.stringify(result, null, 2));
                return;
