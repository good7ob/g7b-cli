...
exports.registerInvitationCommands = void 0;
const ApiClient_1 = __importDefault(require("../../../services/ApiClient"));
function registerInvitationCommands(orgCommand) {
    ...
    orgCommand
        ...
        try {
            ...
            const result = await ApiClient_1.default.get(`/api/v1/orgs/${orgId}/invitations`, params);
            ...
            console.log('─'.repeat(85));
            records.forEach((inv) => {
                console.log(String(inv.id).padEnd(10) +
    ...
    orgCommand
        ...
        try {
            await ApiClient_1.default.post(`/api/v1/orgs/${orgId}/invitations/${invitationId}/revoke`);
    ...
    orgCommand
        ...
        .action(async (orgId, invitationId) => {
        try {
            await ApiClient_1.default.post(`/api/v1/orgs/${orgId}/invitations/${invitationId}/resend`);
    ...
    orgCommand
        ...
        try {
            ...
            const result = await ApiClient_1.default.get('/api/v1/orgs/invitations/mine', params);
            const records = Array.isArray(result) ? result : result?.records || [];
            if (options.json) {
                console.log(JSON.stringify(result, null, 2));
                return;
            ...
            console.log('─'.repeat(80));
            records.forEach((inv) => {
                console.log((inv.orgName || '-').substring(0, 22).padEnd(24) +
                    (inv.role || '-').padEnd(12) +
    ...
    orgCommand
        ...
        .action(async (token) => {
        try {
            const result = await ApiClient_1.default.post(`/api/v1/orgs/invitations/${token}/accept`);
    ...
    orgCommand
        ...
        .action(async (token) => {
        try {
            await ApiClient_1.default.post(`/api/v1/orgs/invitations/${token}/decline`);
            console.log(`✓ 已拒绝邀请`);
        }
        catch (error) {
    ...
    orgCommand
        ...
        .action(async (orgId, options) => {
        try {
            await ApiClient_1.default.post(`/api/v1/orgs/${orgId}/ownership-transfer`, {
...
exports.registerInvitationCommands = registerInvitationCommands;
