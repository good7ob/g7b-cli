...
function riskColor(val, ok, warn, reverse = false) {
    if (reverse)
        return val >= ok ? '🟢' : val >= warn ? '🟡' : '🔴';
    return val <= ok ? '🟢' : val <= warn ? '🟡' : '🔴';
}
function registerReportCommands(qcCommand) {
    ...
    reportCmd
        ...
        try {
            ...
            const d = await ApiClient_1.default.get(`/qc/projects/${projectId}/quality/dashboard`, params);
            if (options.json) {
            ...
            // ── 模块分布 ─────────────────────────────────────────────────────
            const modules = d?.moduleBreakdown ?? [];
            if (modules.length) {
                console.log('\n模块 Bug 分布:');
            ...
            // ── 按严重度修复时间 ──────────────────────────────────────────────
            const bySev = d?.resolveTimeBySeverity ?? [];
            if (bySev.length) {
                console.log('\n平均修复时间（按严重度）:');
            ...
            if (reasons.length) {
                const totalR = reasons.reduce((s, r) => s + (r.bugCount ?? 0), 0);
                console.log('\nBug 原因分布:');
