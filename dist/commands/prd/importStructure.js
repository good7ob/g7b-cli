...
exports.registerImportStructureCommand = void 0;
 ...
 */
const LAYER_CN = { feature: 'Feature', fp: 'FP', rp: 'RP' };
const OP_ICON = { create: '＋', update: '↻' };
function positiveInt(value, flag) {
    const n = Number(value);
    if (!Number.isInteger(n) || n <= 0)
        throw new Error(`${flag} 必须是正整数${value === undefined ? `（缺少 ${flag}，仅 --parse-only 可省略）` : `: ${value}`}`);
    return n;
}
...
const dist = (counts) => Object.entries(counts).map(([k, v]) => `${k} ${v}`).join('  ');
...
function registerImportStructureCommand(prdCommand) {
    prdCommand
        ...
        try {
            ...
            result.actions.forEach((a) => console.log(`  ${OP_ICON[a.op]} ${LAYER_CN[a.layer].padEnd(8)}${a.id}${a.field ? `  ${a.field}: ${a.from} → ${a.to}` : ''}`));
            printCounts(result);
...
exports.registerImportStructureCommand = registerImportStructureCommand;
//# sourceMappingURL=importStructure.js.map